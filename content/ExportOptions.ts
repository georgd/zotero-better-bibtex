...
$patch$(Zotero_File_Interface_Export, 'updateOptions', original => function(options) {
  ...
  switch (translator) {
    case 'b6e39b57-8942-4d11-8259-342c46ce395f':
      textContent = getString('exportOptions.reminder', { translator: 'Better BibLaTeX' })
      break

    case '9cb70025-a888-4a29-a210-93ec52da40d4':
      textContent = getString('exportOptions.reminder', { translator: 'Better BibTeX' })
      break
...
function addEventHandlers() {
  for (const id of [ 'export-option-exportFileData', 'export-option-keepUpdated' ]) {
    ...
    if (reset && (id === 'export-option-keepUpdated')) {
      ...
      node.setAttribute('label', getString('exportOptions.keepUpdated'))
    }
